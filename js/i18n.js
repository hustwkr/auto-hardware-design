...
/* Capacitor — dynamic UI */
    ...
    "filter.subtitle":   { zh: "基于运放的有源低通滤波器 · 一阶差分 / 二阶 MFB · 幅频相频特性 · E24/E12 标准值", en: "Op-amp active low-pass filter · 1st-order Diff / 2nd-order MFB · Bode plot · E24/E12 standard values" },
    ...
    "filter.poleFreq":   { zh: "转折频率", en: "Corner Frequency" },
    ...
    /* Op-amp analysis calculation process (report) */
    "filter.calc.title":      { zh: "计算过程", en: "Calculation Process" },
    "filter.calc.stab":       { zh: "稳定性 / 相位裕度 PM", en: "Stability / Phase Margin" },
    "filter.calc.load":       { zh: "负载电容 C_L 效应", en: "Load Capacitance C_L Effect" },
    "filter.calc.loopFc":     { zh: "环路增益 @ f_c（有限开环精度）", en: "Loop Gain at f_c (finite open-loop accuracy)" },
    "filter.calc.noise":      { zh: "输出噪声", en: "Output Noise" },
    "filter.calc.offset":     { zh: "输出失调", en: "Output Offset" },
    "filter.calc.tol":        { zh: "电阻容差（直流增益）", en: "Resistor Tolerance (DC gain)" },
    "filter.calc.aolModel":   { zh: "单极点开环增益模型", en: "single-pole open-loop model" },
    "filter.calc.betaMfb2":   { zh: "β(f) 用 opamp-removal 法数值求解（移除运放、驱动 v_o=1、输入置零，解被动网络 KCL）", en: "β(f) solved numerically via opamp removal (op-amp removed, drive v_o = 1, inputs grounded, passive-network KCL)" },
    "filter.calc.betaDiff1":  { zh: "β(f) 闭式（反馈分压）：", en: "β(f) closed form (feedback divider):" },
    "filter.calc.ydpNote":    { zh: "Y_dp 为反馈网络在 v_o 节点（输入置零）的驱动点导纳；R_o ≈ V_S/I_sc 为估算值，故 PM 仍为上限估计。", en: "Y_dp = driving-point admittance of the feedback network at v_o (inputs grounded); R_o ≈ V_S/I_sc is an estimate, so PM remains an upper bound." },
    "filter.calc.loopFcNote": { zh: "闭环响应偏离理想滤波器形状约 1/L；L < 25 dB 视为精度不足。", en: "closed-loop deviation from the ideal response ≈ 1/L; L < 25 dB is considered insufficient." },
    "filter.calc.intNote":    { zh: "对数网格梯形积分（0.1 Hz – 10 MHz）", en: "log-grid trapezoidal integration (0.1 Hz – 10 MHz)" },
